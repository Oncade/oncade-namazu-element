...
public class OncadePurchaseHandler extends OncadeWebhookHandler<OncadePurchaseEvent> {
    ...
    public void handleImpl(OncadeRawWebhookPayload rawPayload, Map<String, String> requestData) {
        ...
        try {
            ...
            logger.info("[Webhook] Stored purchase event for purchaseId={}", purchase.getPurchaseId());
        } catch (Exception ex) {
            logger.error("[Webhook] Failed to store purchase event for purchaseId={}: {}",
                    purchase.getPurchaseId(), ex.getMessage(), ex);
        }

        OncadePurchaseEvent event = getEventName();
        if (event == OncadePurchaseEvent.PURCHASES_COMPLETED) {
            try {
                purchaseService.createReceipt(purchase);
                logger.info("[Webhook] Receipt created for purchaseId={}", purchase.getPurchaseId());
            } catch (Exception ex) {
                logger.error("[Webhook] Failed to create receipt for purchaseId={}: {}",
                        purchase.getPurchaseId(), ex.getMessage(), ex);
            }
        } else if (event == OncadePurchaseEvent.PURCHASES_REFUNDED) {
            try {
                purchaseService.revokeReceipt(purchase);
                logger.info("[Webhook] Receipt revoked for purchaseId={}", purchase.getPurchaseId());
            } catch (Exception ex) {
                logger.error("[Webhook] Failed to revoke receipt for purchaseId={}: {}",
                        purchase.getPurchaseId(), ex.getMessage(), ex);
            }
        }
    }
